...
describe("Sigmoid Curator Vault", function () {
  it("Should buy and sell small amounts", async function () {
    ...
    let tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "10", "10", "190");
    expect(tokensPurchased).to.be.equal("10") // Should be 10 

    // Test starting at 0 and buying half and whole
    tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", "10");
    expect(tokensPurchased).to.be.equal("10") // Should be 10

    tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", "200");
    expect(tokensPurchased).to.be.equal("20") // Should be 20 but rounds to 19

    // Try buying past the flattening of the curve
    tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", "400");
    ...
    let saleRefund = await sigmoid.calculatePaymentReturnedFromShares(a, b, c, "20", "200", "10");
    expect(saleRefund).to.be.equal("190") // Should be 190

    // Test starting at full curve and sell all 20
    saleRefund = await sigmoid.calculatePaymentReturnedFromShares(a, b, c, "20", "200", "20");
  ...
  it("Should be able to sell the same amount it bought", async function () {
    ...
    for (let i = 0; i <= 400; i += 20) {
      // Purchase shares with i payment tokens
      let tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", BigNumber.from(i));

      // Sell purchased shares
      let saleRefund = await sigmoid.calculatePaymentReturnedFromShares(a, b, c, tokensPurchased, BigNumber.from(i), tokensPurchased);
  ...
  it("Should buy the max curve with 10^6 decimal token", async function () {
    ...
    for (let i = 0; i <= maxPurchaseAmount; i += increment) {
      ...
      // Purchase shares with i payment tokens
      let tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", priceAmount);

      // Sell purchased shares
      let saleRefund = await sigmoid.calculatePaymentReturnedFromShares(a, b, c, tokensPurchased, priceAmount, tokensPurchased);
  ...
  it("Should buy small amount at start and end of curve", async function () {
    ...
    tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, "0", "0", priceAmount);
    ...
    tokensPurchased = await sigmoid.calculateSharesBoughtFromPayment(a, b, c, tokensPurchased, priceAmount, newPriceAmount);
