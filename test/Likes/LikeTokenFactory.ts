...
import { TEST_LIKE_NFT_URI, TEST_NFT_URI, ZERO_ADDRESS } from "../Scripts/constants";
...
describe("LikeTokenFactory", function () {
  it("Should get initialized", async function () {
    ...
    const { likeTokenImpl, addressManager } = await deploySystem(OWNER);

    const LikeTokenFactoryFactory = await ethers.getContractFactory("LikeTokenFactory");
    const deployedTest1155 = await upgrades.deployProxy(LikeTokenFactoryFactory, [
      addressManager.address,
      likeTokenImpl.address,
      TEST_NFT_URI
    ]);
    const likeTokenFactory = LikeTokenFactoryFactory.attach(deployedTest1155.address);
  ...
  it("Should gate issuance", async function () {
    ...
    await expect(
      likeTokenFactory
        .issueLikeToken(OWNER.address, "1", OWNER.address, "1")
    ...
    await likeTokenFactory.issueLikeToken(OWNER.address, "1", OWNER.address, "1")
  });
  ...
  it("Should deploy Token Contract and mint", async function () {
    ...
    const tokenKey = deriveLikeTokenIndex(
      CHAIN_ID, NFT_ADDRESS, NFT_ID
    )
    ...
    // Issue Token
    const tx = await likeTokenFactory.issueLikeToken(OWNER.address, CHAIN_ID, NFT_ADDRESS, NFT_ID)
