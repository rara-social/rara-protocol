...
describe("RoleManager", function () {
  ...
  it("Should allow owner to set address manager", async function () {
    ...
    await expect(manager.connect(ALICE).grantRole(role, CAROL.address)).to.be
      .reverted;
  ...
  it("Should allow owner to set curator tokens admin", async function () {
    // eslint-disable-next-line no-unused-vars
    const [OWNER, ALICE, BOB, CAROL] = await ethers.getSigners();
    ...
    const role = await manager.CURATOR_TOKEN_ADMIN();
    ...
    // Verify it was set
    result = await manager.isCuratorTokenAdmin(BOB.address);
    expect(result).to.equal(true);
